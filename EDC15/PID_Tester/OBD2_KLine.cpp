...
bool OBD2_KLine::tryFastInit() {
  debugPrintln(F("🔁 Trying ISO14230_Fast (BMW EDC16 Hack)"));

  // Niezawodny hack na Baudrate dla ESP32 omijający błąd '00'
  #if defined(ESP32) || defined(ARDUINO_ARCH_ESP32)
  _serial->updateBaudRate(360);
  _serial->write(0x00);
  _serial->flush();
  delay(22);
  _serial->updateBaudRate(10400);
  #else
  setSerial(false);
  ...
  // Czyszczenie bufora z ewentualnych śmieci
  while(_serial->available()) _serial->read();

  // Ramka StartComm celowana w silnik BMW (Adres 0x12)
  uint8_t bmwInit[] = {0x81, 0x12, 0xF1, 0x81, 0x05};
  for(int i = 0; i < 5; i++) {
    _serial->write(bmwInit[i]);
  }
  _serial->flush();
  
  delay(60); // Czekamy na odpowiedź

  if (!readData()) return false;

  // Szukamy bajtu C1 (Pozytywna odpowiedź) gdziekolwiek w buforze
  for (int i = 0; i < sizeof(resultBuffer); i++) {
    if (resultBuffer[i] == 0xC1) {
      debugPrintln(F("✅ Protocol Detected: ISO14230_Fast (BMW)"));
      connectionStatus = true;
      connectedProtocol = "ISO14230_Fast";
      delay(50); // Timing P2 (Przerwa dla ECU przed następną komendą)
      return true;
    }
  }
...
void OBD2_KLine::writeData(uint8_t mode, uint8_t pid) {
  ...
  // Odpalamy ulepszone czyszczenie echa
  clearEcho();
}
...
void OBD2_KLine::clearEcho() {
  _serial->flush(); // Czekamy, aż mikrokontroler fizycznie wypchnie wszystkie bity
  delay(20);        // Dajemy 20ms na powrót echa sprzętowego
  
  int result = _serial->available();
  if (result > 0) {
    for (int i = 0; i < result; i++) {
      _serial->read(); // Opróżniamy bufor z echa
    }
